...
def export_chart_comments(output_path: Path) -> None:
    ...
    lines.extend(["", "GRAFICI SECONDARI / DA SCARTARE in executive summary:"])
    for chart, note in CHART_REVIEW["secondari_o_da_scartare_in_presentazione_executive"].items():
        lines.append(f"- {chart}: {note}")

    lines.extend(["", "Nota: i grafici secondari restano utili in appendice tecnica o sessioni di approfondimento."])
...
def harmonize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Allinea i nomi colonna tra raw e processed mantenendo retrocompatibilità."""
    df = df.copy()
    rename_map = {
    ...
    if "ChurnValue" in df.columns:
        ...
        # alias legacy usato da alcune funzioni
...
def plot_map_distribution(df, output_path):
    required = {"Longitude", "Latitude", "ChurnValue"}
    if not required.issubset(df.columns):
        print("[plots] Skip map_churn_distribution: missing Longitude/Latitude/ChurnValue")
        return

    plt.figure(figsize=(10, 7))
    sns.scatterplot(x="Longitude", y="Latitude", hue="ChurnValue", data=df, alpha=0.4, palette=PALETTE, s=15)
...
def plot_num_services_count(df, output_path):
    ...
    # Se NumServices esiste già (tipico del preprocessing v2), lo riutilizziamo.
    if "NumServices" not in df.columns:
        ...
        elif "StreamingBundleCount" in df.columns:
            # fallback minimo quando i dettagli streaming sono aggregati
            df["NumServices"] = pd.to_numeric(df["StreamingBundleCount"], errors="coerce").fillna(0)
        else:
            print("[plots] Skip Count_Churn_by_NumServices: missing service columns and NumServices")
            return

    df["NumServices"] = pd.to_numeric(df["NumServices"], errors="coerce").fillna(0).round().astype(int)
...
def plot_outliers_boxplots(df, output_path):
    cols_to_plot = ["TenureMonths", "MonthlyCharges", "TotalCharges", "Avg Monthly Spend", "Charges per Service"]
...
def plot_churn_heatmap_grid(df, output_path):
    ...
    heatmap_data = df.pivot_table(index="Charges_Bin", columns="Tenure_Bin", values="Churn_Numeric", aggfunc="mean")
